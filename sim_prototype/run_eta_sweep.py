...
def _run_point(label, knob, value, M, seed0, t_stop=T_STOP):
    ...
    if knob == "eta":
        ctx, cfg = _ctx(make_system(eta=value)), make_cfg()
    elif knob == "dt":
        ctx, cfg = _ctx(make_system()), make_cfg(dt=value)
    elif knob == "wdelta":
        ctx, cfg = _ctx(make_system()), make_cfg(wdelta=value)
    else:
    ...
    for s in range(seed0, seed0 + M):
        if s in done:
            continue
        conf, ts, tf = run_path(ctx, cfg, RHO0, seed=s, t_stop=t_stop)
        rec["seeds"].append(int(s))
        rec["shell"].append(ts is not None)
        rec["funnel"].append(not conf)
        rec["tau_shell"].append(None if ts is None else float(ts))
        rec["tau_funnel"].append(None if tf is None else float(tf))
    db[label] = rec
    _save(db)
...
def validate(seeds=(0, 1, 2, 7), buffers=(0.20, 0.25)):
    ...
    for sb in buffers:
        ...
        for s in seeds:
            ...
            worst = max(worst, 0.0)  # placeholder; path-level check below
        # one path-level numeric comparison per buffer
